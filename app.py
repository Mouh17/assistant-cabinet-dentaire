...
ROLE = ("Tu es l'assistant du Cabinet Dentaire Belle Sourire, à Paris. "
        "Tu réponds aux questions des patients de façon précise, utile et directe.")

DOSSIER_DOCUMENTS = "documents"  # tes PDF vivent dans ce dossier, à la racine du dépôt


# ----------------------------------------------------------------------
# 2. Chargement des documents — PDF stockés localement dans documents/
#    (plus de dépendance à un dataset externe : ajoute/retire tes PDF
#    dans ce dossier et redéploie, c'est tout)
# ----------------------------------------------------------------------
def charger_documents():
    textes = {}
    for f in sorted(os.listdir(DOSSIER_DOCUMENTS)):
        if not f.lower().endswith(".pdf"):
            continue
        chemin = os.path.join(DOSSIER_DOCUMENTS, f)
        try:
...
for f, t in TEXTES.items():
    nom = f.replace(".pdf", "").replace("cabinet-", "Cabinet ").replace("-", " ").title()
